...
date = "2024-02-13" #todays_date, 2024-02-13
pull_data = False #True # False
market_open = "default" 
market_close = "default"

tickerdata_open = "default" 
ticketdata_close = "default"
...
# Prepare array for LSTM
def prepare_data_for_lstm_array(array):
    pdb.set_trace()
    # prices = np.array(prices).reshape(-1, 1)
    # scaler = MinMaxScaler(feature_range=(0, 1))
    # scaled_prices = scaler.fit_transform(prices)
...
directory_path = './new_datasets/' + str(date) + "/"

# Output file path
output_file_path = './prediction_summary' + str(date) + '.csv'
...
for filename in os.listdir(directory_path):
    if filename.endswith('.csv'):
        ...
        # Prepare data for LSTM model
        # hourly_prices = df['Price'].to_numpy()
        hourly_prices = df[['v','vw','o','h','c','n']].to_numpy()
        X, y, scaler = prepare_data_for_lstm(hourly_prices)
        X = X.reshape((X.shape[0], X.shape[1], 1))
        X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.1, random_state=42)
